...
import { Heading, Text, Card, Checkbox, ArrowDropDownIcon, Toggle } from '@apeswapfinance/uikit'
...
const PoolMonkey = styled.div`
  ...
  background-image: url('../../../images/earnpassive.0b5a2004.png');
  width: 300px;
  height: 100%;
...
const StyledLabel = styled.div<LabelProps>`
  ...
  background-image: ${({ active, theme }) => active && theme.colors.gradients.ltr};
`
 ...
 const { t } = useTranslation();
  ...
  return (
    <>
      <Header>
        <HeadingContainer>
          <StyledHeading as="h1" mb="8px" mt={0} color="primary" fontWeight={700}>
            Champagne
          </StyledHeading>
          {size.width > 968 && (
            <StyledSubHeading fontSize="22px" fontWeight={400} color="text">
              {t('Stake CORK tokens to more new token.')} <br /> {t('High APR, low risk.')}
            </StyledSubHeading>
          )}
        </HeadingContainer>
        {/* <MonkeyWrapper>
            ...
            <ButtonCheckWrapper>
              ...
              <ToggleContainer size={size.width}>
                <ToggleWrapper onClick={() => setStakedOnly(!stakedOnly)}>
                  <Toggle checked={stakedOnly} onChange={() => setStakedOnly(!stakedOnly)} sizes="sm" />
